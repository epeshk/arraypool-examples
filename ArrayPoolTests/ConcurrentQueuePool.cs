...
public sealed partial class ConcurrentQueuePool<T> : ArrayPool<T>
{
  ...
  private sealed class PerCoreLockedStacks
  {
    ...
    private readonly ConcurrentQueue<T[]>[] _perCoreStacks;
    ...
    public PerCoreLockedStacks()
    {
      ...
      var stacks = new ConcurrentQueue<T[]>[s_lockedStackCount];
      for (int i = 0; i < stacks.Length; i++)
      {
        stacks[i] = new ConcurrentQueue<T[]>();
      }

...
public sealed partial class BoundedQueuePool<T> : ArrayPool<T>
{
  ...
  [ThreadStatic] private static ThreadLocalArray[]? t_tlsBuckets;
  ...
  private sealed class PerCoreLockedStacks
  {
    ...
    private readonly BoundedConcurrentQueue<T[]>[] _perCoreStacks;
    ...
    public PerCoreLockedStacks()
    {
      ...
      var stacks = new BoundedConcurrentQueue<T[]>[s_lockedStackCount];
      for (int i = 0; i < stacks.Length; i++)
      {
        stacks[i] = new BoundedConcurrentQueue<T[]>(8);
      }
    ...
    public bool TryPush(T[] array)
    {
      ...
      BoundedConcurrentQueue<T[]>[] stacks = _perCoreStacks;
